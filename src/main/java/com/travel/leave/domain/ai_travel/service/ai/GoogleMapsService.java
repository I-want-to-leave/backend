...
public class GoogleMapsService {
    ...
    // 특정 장소의 좌표 데이터를 가져온다.
    public CompletableFuture<ResponseEntity<JsonNode>> fetchCoordinatesResponse(String placeName) {
    ...
    public CompletableFuture<LatLngDTO> getCoordinates(String placeName, int stepOrder, Timestamp startAt, Timestamp endAt, TransportType transportType) {
        return fetchCoordinatesResponse(placeName)
                .thenCompose(response -> mapToLatLngDTO(response.getBody(), placeName, stepOrder, startAt, endAt,transportType))
                .exceptionally(ex -> null); // 매핑되지 못한 건 추가하면 안 됨
    }
